...
import { FileText, Mic, Calendar, Trash2, Loader2 } from 'lucide-react';
        ...
        {loading ? (
            ...
            <div className="grid gap-4">
                {history.length === 0 ? (
                    ...
                    history.map((item) => (
                        <Card key={item.id} className="bg-black/40 backdrop-blur-xl border-white/10 hover:bg-white/5 transition-colors group">
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <Button 
                                        variant="ghost" 
                                        size="icon" 
                                        className="h-8 w-8 text-red-500/50 hover:text-red-500 hover:bg-red-500/10 opacity-0 group-hover:opacity-100 transition-opacity"
                            ...
                            <CardContent className="pt-4">
                                <div className="flex gap-4">
                                    {item.preview_url && (
                                        <img src={item.preview_url} alt="Thumb" className="h-16 w-16 object-cover rounded-md border border-white/10" />
                                    )}
